...
fun MatchValidityScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text(text = stringResource(Res.string.match_validity_title)) },
                navigationIcon = {
        ...
        when {
            uiState.isLoading -> {
                Text(
                    text = stringResource(Res.string.summary_loading),
                    modifier = Modifier
                        .fillMaxSize()
            ...
            uiState.loadFailed || audit == null -> {
                Text(
                    text = stringResource(Res.string.summary_load_error),
                    modifier = Modifier
                        .fillMaxSize()
                        .padding(innerPadding)
                        .padding(24.dp),
...
private fun MatchValidityBody(
    ...
    LazyColumn(
        ...
        item {
            ...
            Text(
                text = stringResource(Res.string.match_validity_subtitle),
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(modifier = Modifier.height(8.dp))
            val statusText: String = if (audit.allMatchesValid) {
                stringResource(Res.string.match_validity_all_ok)
            } else {
                stringResource(Res.string.match_validity_some_issues)
            }
        ...
        if (audit.rounds.isEmpty()) {
            item {
                Text(
                    text = stringResource(Res.string.match_validity_empty),
                    style = MaterialTheme.typography.bodyMedium,
                )
            }
        } else {
            audit.rounds.forEach { section: RoundValiditySection ->
                item(key = "round_header_${section.roundNumber}") {
                    Text(
                        text = stringResource(
                            Res.string.match_validity_round_header,
                            section.roundNumber,
                            audit.rounds.size,
                        ),
                        style = MaterialTheme.typography.titleSmall,
...
private fun MatchValidityCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            Text(
                text = stringResource(Res.string.match_validity_court_label, row.courtNumber),
                style = MaterialTheme.typography.labelLarge,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(modifier = Modifier.height(6.dp))
            Text(
                text = stringResource(Res.string.match_validity_team_vs, row.teamAPairLabel, row.teamBPairLabel),
                style = MaterialTheme.typography.bodyLarge,
            )
            Spacer(modifier = Modifier.height(6.dp))
            val scoreLine: String = if (row.isScoreEntered) {
                stringResource(Res.string.match_validity_score_line, row.scoreA, row.scoreB)
            } else {
                stringResource(Res.string.match_validity_score_not_entered)
            }
            Text(
                text = stringResource(Res.string.match_validity_score_label, scoreLine),
                style = MaterialTheme.typography.bodyMedium,
            )
            if (row.issues.isNotEmpty()) {
                ...
                Text(
                    text = stringResource(Res.string.match_validity_row_ok),
                    style = MaterialTheme.typography.bodySmall,
...
private fun issueText(issue: MatchValidityIssue, maxCombinedMatchScore: Int): String {
    return when (issue) {
        MatchValidityIssue.INVALID_PLAYER_INDEX -> stringResource(Res.string.match_validity_issue_bad_index)
        MatchValidityIssue.DUPLICATE_PLAYERS_ON_COURT -> {
            stringResource(Res.string.match_validity_issue_duplicate_court)
        }
        MatchValidityIssue.SAME_PLAYER_TWICE_ON_TEAM_A -> {
            stringResource(Res.string.match_validity_issue_team_a_duplicate)
        }
        MatchValidityIssue.SAME_PLAYER_TWICE_ON_TEAM_B -> {
            stringResource(Res.string.match_validity_issue_team_b_duplicate)
        }
        MatchValidityIssue.NEGATIVE_SCORE -> stringResource(Res.string.match_validity_issue_negative_score)
        MatchValidityIssue.COMBINED_SCORE_EXCEEDS_LIMIT -> {
            stringResource(Res.string.match_validity_issue_combined_exceeds, maxCombinedMatchScore)
        }
    }
}
