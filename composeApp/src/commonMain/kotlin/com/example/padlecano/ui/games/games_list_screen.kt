...
fun GamesListScreen(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text(text = stringResource(Res.string.games_title)) },
                actions = {
                    TextButton(
                        onClick = { showDeleteAllDialog = true },
                        enabled = uiState.tournaments.isNotEmpty(),
                    ) {
                        Text(text = stringResource(Res.string.games_delete_all))
                    }
                },
            )
        },
        floatingActionButton = {
            FloatingActionButton(onClick = onCreateGameClick) {
                Icon(
                    ...
                    contentDescription = stringResource(Res.string.games_fab_new_game),
                )
            }
        },
    ) { innerPadding ->
        if (showDeleteAllDialog) {
            AlertDialog(
                ...
                title = { Text(text = stringResource(Res.string.games_delete_all_confirm_title)) },
                text = { Text(text = stringResource(Res.string.games_delete_all_confirm_message)) },
                confirmButton = {
                    TextButton(
                        onClick = {
                        ...
                        Text(text = stringResource(Res.string.games_delete_all_confirm))
                    }
                },
                dismissButton = {
                    TextButton(onClick = { showDeleteAllDialog = false }) {
                        Text(text = stringResource(Res.string.games_delete_all_cancel))
                    }
                },
            )
        }
        ...
        if (pendingDelete != null) {
            val titleText: String = if (pendingDelete.title.isBlank()) {
                stringResource(Res.string.tournament_untitled)
            } else {
                pendingDelete.title
            }
            AlertDialog(
                onDismissRequest = { tournamentPendingDelete = null },
                title = { Text(text = stringResource(Res.string.games_delete_one_confirm_title)) },
                text = {
                    Text(
                        text = stringResource(
                            Res.string.games_delete_one_confirm_message,
                            titleText,
                        ),
                    )
                },
                confirmButton = {
                    TextButton(
                        ...
                        Text(text = stringResource(Res.string.games_delete_one_confirm))
                    }
                },
                dismissButton = {
        ...
        if (uiState.tournaments.isEmpty()) {
            Column(
                ...
                Text(
                    text = stringResource(Res.string.games_empty_hint),
...
private fun TournamentRow(
    ...
    val statusLabel: String = when (tournament.status) {
        TournamentStatus.DRAFT -> stringResource(Res.string.tournament_status_draft)
        TournamentStatus.ACTIVE -> stringResource(Res.string.tournament_status_active)
        TournamentStatus.FINISHED -> stringResource(Res.string.tournament_status_finished)
    }
    Card(
        ...
        Row(
            ...
            IconButton(onClick = onDeleteClick) {
                Icon(
                    ...
                    contentDescription = stringResource(Res.string.content_description_delete_tournament),
                )
            }
        }
    }
