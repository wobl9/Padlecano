...
class AmericanoScheduleGeneratorTest {
    ...
    fun generate_prefersSpacedSecondOpponentMeetings_forEightPlayers() {
        ...
        assertTrue(spacedRematchCount > 0, "Expected at least one spaced opponent rematch for 8 players")
        assertTrue(spacedRematchCount >= consecutiveRematchCount, "Expected spaced rematches to outnumber consecutive rematches")
    }
    ...
    fun generate_verifyScheduleAgainstIdeal_passesForGeneratedSchedule() {
        for (playerCount: Int in listOf(4, 8, 12, 16)) {
            ...
            assertTrue(report.allOk, "Schedule verification failed for $playerCount players")
        }
    }
    ...
    private fun assertMinimumOpponentCoverage(playerCount: Int) {
        ...
        for (playerA: Int in 0 until playerCount) {
            for (playerB: Int in playerA + 1 until playerCount) {
                assertTrue(opponentCounts[playerA][playerB] >= 1, "Players $playerA and $playerB should face each other in $playerCount-player event")
            }
        }
    }
